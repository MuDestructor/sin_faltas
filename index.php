...
$landingExampleURL = "https://www.eldiario.es/sociedad/espana-registra-dato-diario-contagios-pandemia_1_6746127.html";
...
if (empty($url)) {
    if (empty($startURL)) {
      die("<html><head><title>miniProxy</title></head><body><h1>Bienvenido a sinFaltasProxy!</h1>sinFaltasProxy puede invocarse directamente: <a href=\"" . PROXY_PREFIX . $landingExampleURL . "\">" . PROXY_PREFIX . $landingExampleURL . "</a><br /><br />OO simplemente pegando una URL aquí:<br /><br /><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"¡Carga!\" /></form></body></html>");
    } else {
      $url = $startURL;
    }
